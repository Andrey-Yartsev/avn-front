...
export default {
  ...
  computed: {
    ...
    initItems() {
      return this.state.blockedPosts;
    },
    items() {
      let users = this.state.blockedPosts;
  ...
  methods: {
    ...
    unblock(userId) {
      this.$store
        .dispatch(this.storePath + "/unblockPosts", userId)
        .then(() => {
          this.$store.dispatch(this.storePath + "/fetchBlockedPosts");
        });
    }
  },
