<template>
  <div class="SettingsNotificationsView">
    <form v-on:submit.stop.prevent="saveData">
          ...
          <template v-else>
            ...
            >
              <button
                ...
                :disabled="!isCategoriesChanges || loading"
              >
                Save changes
              </button>
            </div>
...
export default {
  ...
  computed: {
    ...
    isCategoriesChanges() {
      if (!this.user.nominationCategories && !this.category["0"]) {
        return false;
      }
      return (
        JSON.stringify(this.category) !==
        JSON.stringify(
          JSON.parse(this.user.nominationCategories)?.[this.eventId]
        )
      );
    }
  },
  ...
  methods: {
    ...
    saveData() {
      this.save({ shouldCheckChanges: false });
    }
  },
  created() {
    this.init();
